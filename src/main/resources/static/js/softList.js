...
function list() {
    ...
    $.ajax({
        url:  "../map/text_list",
        type:"post",
        data:{cid:cid,hiv:hiv,pageNum:pageNum,siteType:3},
        success:function (result) {
            var lis=[];
            ...
            if(result.size==0){
                ...
                $.each(result.list,function (key,value){
                    var date=format(value.addTime,'yyyy-MM-dd')
                    // console.info(date)
                    lis.push(
                       '<li>\n' +
                        '\t\t\t\t\t                    <a title="'+value.title+'" href="../index/soft_detailed?id='+value.id+' ">\n' +
                        '\t\t\t\t\t                        <div class="img-box"><img src="'+value.thumbnailImg+'"></div>\n' +
                        '\t\t\t\t\t                        <div class="info">\n' +
                        '\t\t\t\t\t                            <h3>'+value.title+'</h3>\n' +
                        '\t\t\t\t\t                            <p>\n' +
                        '\t\t\t\t\t                                <strong>作者：'+value.author+'</strong>\n' +
                        '\t\t\t\t\t                                <span>'+date+'</span>\n' +
                        '\t\t\t\t\t                            </p>\n' +
                        '\t\t\t\t\t                        </div>\n' +
                        '\t\t\t\t\t                    </a>\n' +
